  ...
  const handleContinueAppPress = async () => {
    ...
    try {
      const response = await axiosInstance.post(
        `${process.env.EXPO_PUBLIC_ID_SERVER_URL}/auth/local/complete-signup`,
        {
          username: username,
        },
        {
      ...
      <View className="flex px-6 w-full h-screen">
        <View className="flex flex-col space-y-3 mt-12">
          ...
          <Text className="text-xl text-gray-400">
            Enter a username for your account below.
          </Text>
        </View>
        ...
        >
          <View className="flex flex-row w-full px-4 items-center mx-auto">
            {waiting ? (
              <ActivityIndicator size={"small"} className="text-white" />
            ) : (
              <>
                <Text className="text-white font-medium text-lg left-0 mr-auto">
                  Continue to App
                </Text>
