...
export class CardSetController {
    ...
    async createSet(
        ...
        if (!description) throw new BadRequestException("Please provide a description.");
        if (!cards || cards.length < 2) throw new BadRequestException("To create a cardset we need at least 2 cards.");

        const userId = req.user.id;

        const result = await this.cardSetService.createCard(userId, title, description, cards);
