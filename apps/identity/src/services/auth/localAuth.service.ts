...
export class LocalAuthService {
  ...
  async completeSignup(userId: string, username: string) {
    try {
      ...
      if (query)
        throw new InternalServerErrorException('This username is taken!');
  ...
  async isEmailAvailable(email: string): Promise<boolean> {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'An edge case error occured. Please try again',
      );
    } catch (error) {
  ...
  async authenticateLocalCredentials(email: string, password: string) {
    ...
    try {
      ...
      if (!matchQ)
        throw new NotFoundException(
          'The credentials provided did not match an account stored on our server.',
        );
