...
export class AuthController {
  ...
  async googleAuthPost(
    ...
    if (!idToken || !accessToken)
      throw new NotFoundException('Id token and Access token are required.');
  ...
  @ApiOkResponse({
    type: User,
    description: 'Token is marked as validated and a user object is returned.',
  })
  @ApiCreatedResponse({
    type: User,
    description: 'Token is marked as validated and a user object is returned.',
  ...
  @ApiUnauthorizedResponse({
    type: UE,
    description: 'Unauthorized, user is not logged in or session expired.',
  })
