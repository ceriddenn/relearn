...
export class LocalAuthController {
    ...
    async completeSignup(@Request() req, @Body('username') username) {
        if (!username) throw new NotFoundException("No username provided. Please provide a username with your request.");
    ...
    async signupLocalAccount(@Body('name') name, @Body('email') email, @Body('password') password, @Body('tos') tos) {
        if (!email || !name || !password) throw new NotFoundException("Name, email, and password are required.");
